...
## Overview
- **Product name:** `cdp_curated_evaluation`
- **Layer:** `curated`
- **Domain:** `Not provided`
...
The CLI that ran to create this repo performed the following actions for the given `cdp_curated_evaluation` and `curated`:
...
- Created an **Account level group** with read access for consumers: `cdp-curated-cdp_curated_evaluation-readers`. This group is intended to be the standard consumer group for granting read access to the product in UC.
...
- **`id`** *(Required)* — unique technical name of the data product (e.g., cdp_curated_evaluation). Was set automatically!
...
- **Access & Consumer Group** *(Required)* — group in the format cdp-curated-cdp_curated_evaluation-readers, description of how to request access.
...
- Group created: `cdp-curated-evaluation-readers` — add consumers here to grant read access to UC objects for this product.
...
- [ ] Confirm that `cdp-curated-evaluation-readers` group exists and members know how to request access
